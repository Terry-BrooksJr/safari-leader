...
both the happy paths and every documented failure branch of ``WeatherMan``.
"""
...
class GetClientIpTest(SimpleTestCase):
    ...
    def test_returns_none_when_no_ip_available(self):
        request = self._request({})
        self.assertIsNone(WeatherMan.get_client_ip(request))
...
class GetGeographicDataTest(SimpleTestCase):
    ...
    def test_queries_ipwhois_endpoint_with_ip(self, mock_requests):
        mock_requests.get.return_value = _response(200, self.DATA)
        WeatherMan.get_geographic_data("8.8.8.8")
        mock_requests.get.assert_called_once_with("https://ipwho.is/8.8.8.8")
    ...
    def test_returns_empty_dict_for_loopback_ip(self, mock_requests):
        mock_requests.get.return_value = _response(200, self.DATA)
        self.assertEqual(WeatherMan.get_geographic_data("127.0.0.1"), {})

    def test_returns_empty_dict_for_unspecified_ip(self, mock_requests):
        mock_requests.get.return_value = _response(200, self.DATA)
        self.assertEqual(WeatherMan.get_geographic_data("0.0.0.0"), {})

    def test_returns_empty_dict_for_localhost(self, mock_requests):
        mock_requests.get.return_value = _response(200, self.DATA)
...
class GetForecastTest(SimpleTestCase):
    ...
    def test_missing_coordinate_key_raises_key_error(self):
        # get_geographic_data returns {} on failure and get_forecast does not
        # guard against missing keys, so a KeyError surfaces. This documents the
        # current (unguarded) behavior rather than endorsing it.
        with self.assertRaises(KeyError):
            WeatherMan.get_forecast({})
...
class GetForecastUrlTest(SimpleTestCase):
    ...
    def test_queries_weather_points_endpoint(self, mock_requests):
        mock_requests.get.return_value = _response(200, self.POINTS)
...
class GetWeatherDataTest(SimpleTestCase):
    ...
    def test_requests_the_supplied_forecast_url(self, mock_requests):
        mock_requests.get.return_value = _response(200, self.PAYLOAD)
        WeatherMan.getWeatherData(self.GEO, "https://api.weather.gov/hourly")
        mock_requests.get.assert_called_once_with("https://api.weather.gov/hourly")

    def test_returns_none_when_request_raises(self, mock_requests):
        mock_requests.get.side_effect = ConnectionError("network down")
        self.assertIsNone(
            WeatherMan.getWeatherData(self.GEO, "https://api.weather.gov/hourly")
        )

    def test_returns_none_on_malformed_payload(self, mock_requests):
        # Missing "periods" raises inside the try block and is swallowed.
        mock_requests.get.return_value = _response(200, {"properties": {}})
        self.assertIsNone(WeatherMan.getWeatherData(self.GEO, "url"))
...
class ForecastOrchestrationTest(SimpleTestCase):
    """Verify ``forecast`` wires the helper methods together correctly."""
